...
### shadcn/ui 生成コンポーネントのテスト

- 生成物であることだけを理由に、一律でテストを追加・省略しない。
- リポジトリ固有の変更がある場合、または状態・操作・アクセシビリティに意味のある分岐を持つ場合は、`packages/ui` の実装と同じ階層に Vitest + React Testing Library の `*.test.tsx` を追加する。
- 変更していない単純な表示コンポーネントはテストを省略できる。その場合は、追加・更新する PR に省略理由を記載する。
- `packages/ui` のテストは、再利用可能なコンポーネント自身の状態遷移と公開 DOM・アクセシビリティ契約を検証する。`apps/web` のテストは、アプリ固有の構成、ルーティング、データ連携を検証し、画面をまたぐフローは Playwright E2E で扱う。
