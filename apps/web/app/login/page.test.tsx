...
describe("LoginPage", () => {
  ...
  it("Google ボタンで signIn.social が web オリジンの絶対 URL つきで呼ばれる", async () => {
    mocks.signInSocial.mockResolvedValue({
      data: { url: "https://example.com", redirect: true },
      error: null,
    })
    render(<LoginPage />)
    ...
    expect(mocks.signInSocial.mock.calls[0]?.[0]).not.toHaveProperty(
      "requestSignUp"
    )
  })
