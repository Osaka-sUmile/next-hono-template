...
function createSurvey(): FeedbackSurveyEntity {
  return FeedbackSurveyEntity.reconstitute("survey-1", "pmf-2026", "PMF アンケート", true, [
    FeedbackQuestionEntity.reconstitute("q-1", "single_choice", "どう思いますか？", true, 0, [
      FeedbackChoice.reconstitute("c-1", "very_disappointed", "非常に残念", 0),
    ]),
    FeedbackQuestionEntity.reconstitute("q-2", "text", "一番の価値は？", false, 1, []),
  ]);
}
...
function createDeps(
  ...
  const findActive = overrides.findActive ?? vi.fn().mockResolvedValue(createSurvey());
  // 既定では save をエコーにして、UseCase が「保存結果」を返していることを検証できるようにする。
  const save = overrides.save ?? vi.fn().mockImplementation(async (entity) => entity);
  const generate = overrides.generate ?? vi.fn().mockReturnValue("submission-1");

  const surveyRepository = { findActive } as unknown as IFeedbackSurveyRepository;
  const submissionRepository = { save, findById: vi.fn() } as unknown as IFeedbackSubmissionRepository;
  const idGenerator = { generate } as unknown as IIdGenerator;

  return { surveyRepository, submissionRepository, idGenerator, findActive, save, generate };
}
...
describe("SubmitFeedbackUseCase", () => {
  ...
  it("uses the id from the generator rather than any client-supplied value", async () => {
    const deps = createDeps({ generate: vi.fn().mockReturnValue("generated-id") });
    const useCase = new SubmitFeedbackUseCase(
      deps.surveyRepository,
      deps.submissionRepository,
      deps.idGenerator,
  ...
  it("throws ActiveFeedbackSurveyNotFoundError and does not save when no survey is active", async () => {
    ...
    await expect(useCase.execute({ userId: "user-1", answers: [] })).rejects.toBeInstanceOf(
  ...
  it("propagates the domain error when a required answer is missing and does not save", async () => {
    ...
    await expect(
      useCase.execute({ userId: "user-1", answers: [{ questionId: "q-2", textValue: "value" }] }),
