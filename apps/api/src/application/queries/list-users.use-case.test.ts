...
const buildQueryResult = (overrides: Partial<UserQueryResult> = {}): UserQueryResult => ({
  id: "u1",
...
const createQueryService = (findAll: ReturnType<typeof vi.fn>): IUserQueryService =>
...
describe("ListUsersUseCase", () => {
  it("findAll の結果を UserResponseDto の配列に変換して返す", async () => {
    ...
    const useCase = new ListUsersUseCase(createQueryService(vi.fn().mockResolvedValue(results)));
  ...
  it("ユーザーが存在しない場合は空配列を返す", async () => {
    const useCase = new ListUsersUseCase(createQueryService(vi.fn().mockResolvedValue([])));
