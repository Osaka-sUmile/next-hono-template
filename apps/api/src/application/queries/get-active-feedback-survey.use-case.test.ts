...
import type { FeedbackSurveyView, IFeedbackQueryService } from "@workspace/domain";
...
describe("GetActiveFeedbackSurveyUseCase", () => {
  it("returns the active survey as a response DTO", async () => {
    ...
    const useCase = new GetActiveFeedbackSurveyUseCase(createQueryService({ findActiveSurveyView }));
  ...
  it("throws ActiveFeedbackSurveyNotFoundError when no survey is active", async () => {
    const useCase = new GetActiveFeedbackSurveyUseCase(
      createQueryService({ findActiveSurveyView: vi.fn().mockResolvedValue(null) }),
    );

    await expect(useCase.execute()).rejects.toBeInstanceOf(ActiveFeedbackSurveyNotFoundError);
  });
});
