...
import type { FeedbackSummaryTallyResult, IFeedbackQueryService } from "@workspace/domain";
...
describe("SummarizeFeedbackUseCase", () => {
  it("summarizes the given survey and returns the surveyId with the tallies", async () => {
    ...
    const useCase = new SummarizeFeedbackUseCase(createQueryService({ summarize }));
  ...
  it("returns an empty summary when the survey has no submissions", async () => {
    const useCase = new SummarizeFeedbackUseCase(
      createQueryService({
        summarize: vi.fn().mockResolvedValue({ respondentCount: 0, tallies: [] }),
      }),
    );

    const result = await useCase.execute({ surveyId: "survey-9" });

    expect(result).toEqual({ surveyId: "survey-9", respondentCount: 0, tallies: [] });
  });
});
