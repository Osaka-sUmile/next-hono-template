...
describe("envSchema", () => {
  ...
  it("rejects API origins with a path or query", () => {
    ...
    expect(() =>
      envSchema.parse({
        ...validEnv,
        API_BASE_URL: "https://api.example.com?x=1",
      })
    ).toThrow()
    expect(() =>
      envSchema.parse({
        ...validEnv,
        API_BASE_URL: "https://api.example.com#fragment",
      })
    ).toThrow()
  })
  ...
  it("normalizes WEB_BASE_URL to an origin and rejects a fragment", () => {
    expect(
      envSchema.parse({ ...validEnv, WEB_BASE_URL: "https://web.example.com/" })
        .WEB_BASE_URL
    ).toBe("https://web.example.com")
    expect(() =>
      envSchema.parse({
        ...validEnv,
        WEB_BASE_URL: "https://web.example.com#fragment",
      })
    ).toThrow()
  })
  ...
  it("validates optional SENTRY_DSN", () => {
    ...
    ).toBe("https://sentry.example.com/123")
  ...
  it("validates the sender email format", () => {
    expect(() =>
      envSchema.parse({ ...validEnv, RESEND_FROM_EMAIL: "not-an-email" })
    ).toThrow()
  })
})
