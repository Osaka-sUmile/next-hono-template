...
describe("ErrorSchema", () => {
  it("requires the established error body and accepts only ErrorCodes", () => {
    ...
    expect(
      ErrorSchema.safeParse({ code: ErrorCodes.SESSION_INVALID }).success
    ).toBe(false)
    expect(
      ErrorSchema.safeParse({ error: "Unknown", code: "UNKNOWN" }).success
    ).toBe(false)
  })
  ...
  it("is emitted as the named Error OpenAPI schema", () => {
    ...
    app.openapi(
      ...
      (c) =>
        c.json({ error: "Invalid", code: ErrorCodes.VALIDATION_ERROR }, 400)
    )
    ...
    expect(document.components?.schemas?.Error).toMatchObject({
      ...
      properties: expect.objectContaining({
        ...
        code: expect.objectContaining({
          enum: expect.arrayContaining(Object.values(ErrorCodes)),
        }),
      }),
    })
  })
