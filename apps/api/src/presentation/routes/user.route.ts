...
export const UserSchema = z
  .object({
    ...
    displayName: z.string().nullable().openapi({ description: "User display name (user-settable)" }),
    image: z.string().nullable().openapi({ description: "User avatar URL" }),
    emailVerified: z.boolean().openapi({ description: "Whether the email address has been verified" }),
...
const UpdateUserMeBodySchema = z.object({
  displayName: z
    ...
    .openapi({ description: "New display name. Send null (or an empty string) to clear it." }),
});
...
export const updateUserMeRoute = createRoute({
  ...
  security: [{ cookieAuth: [] }],
  request: {
    body: { required: true, content: { "application/json": { schema: UpdateUserMeBodySchema } } },
  },
  responses: {
    200: { description: "Updated profile", content: { "application/json": { schema: UserProfileSchema } } },
    ...errorResponses({
