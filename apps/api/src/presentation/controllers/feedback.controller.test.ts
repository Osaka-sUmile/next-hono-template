...
import { InvalidArgumentError, UnknownFeedbackQuestionError } from "@workspace/domain";
  ...
  questions: [
    {
      ...
      required: true,
      sortOrder: 0,
      choices: [{ value: "very_disappointed", label: "非常に残念", sortOrder: 0 }],
    },
  ],
};
...
describe("GET /api/v1/feedback/survey", () => {
  ...
  it("returns 404 when no survey is active", async () => {
    const { app } = createTestApp({
      getSession: vi.fn().mockResolvedValue(userSession),
      getFeedbackSurvey: vi.fn().mockRejectedValue(new ActiveFeedbackSurveyNotFoundError()),
    });
...
describe("POST /api/v1/feedback/submissions", () => {
  ...
  it("returns 201 and passes the answers with the session user id", async () => {
    ...
    const res = await app.request(
      postSubmission({ answers: [{ questionId: "q-1", choiceValue: "very_disappointed" }] }),
    );
  ...
  it("rejects a body carrying userId so the submitter cannot be spoofed", async () => {
    ...
    const res = await app.request(
      postSubmission({ userId: "victim-1", answers: [{ questionId: "q-1", textValue: "x" }] }),
    );
  ...
  it("returns 400 without calling the use case when a free-text answer exceeds 2000 characters", async () => {
    ...
    const res = await app.request(
      postSubmission({ answers: [{ questionId: "q-1", textValue: "あ".repeat(2001) }] }),
    );
  ...
  it("accepts a free-text answer of exactly 2000 characters", async () => {
    ...
    const res = await app.request(
      postSubmission({ answers: [{ questionId: "q-1", textValue: "あ".repeat(2000) }] }),
    );
  ...
  it("returns 400 FEEDBACK_INVALID_ANSWER when the answers violate the survey contract", async () => {
    const { app } = createTestApp({
      getSession: vi.fn().mockResolvedValue(userSession),
      submitFeedback: vi.fn().mockRejectedValue(new UnknownFeedbackQuestionError("q-9")),
    });
  ...
  it("returns 404 when no survey is active", async () => {
    const { app } = createTestApp({
      getSession: vi.fn().mockResolvedValue(userSession),
      submitFeedback: vi.fn().mockRejectedValue(new ActiveFeedbackSurveyNotFoundError()),
    });
    ...
    expect(res.status).toBe(404);
    expect((await errorBody(res)).code).toBe(ErrorCodes.FEEDBACK_SURVEY_NOT_FOUND);
  });
  ...
  it("returns 500 for a domain error that is not an answer-contract violation", async () => {
    ...
    const { app } = createTestApp({
      getSession: vi.fn().mockResolvedValue(userSession),
      submitFeedback: vi.fn().mockRejectedValue(new InvalidArgumentError("corrupt survey row")),
    });
...
describe("GET /api/v1/admin/feedback/submissions", () => {
    ...
    items: [
      {
        id: "submission-1",
        surveyId: "survey-1",
        user: { id: "user-1", email: "user@example.com", name: "User", displayName: null },
        createdAt: new Date("2026-07-27T00:00:00.000Z"),
        answers: [
          {
            questionId: "q-1",
  ...
  it("returns 200 with the default paging for an admin", async () => {
    ...
    expect(listFeedbackSubmissions).toHaveBeenCalledWith({ limit: 20, offset: 0 });
  });

  it("passes limit, offset and surveyId through", async () => {
    const { app, listFeedbackSubmissions } = createTestApp({
      getSession: vi.fn().mockResolvedValue(adminSession),
      listFeedbackSubmissions: vi.fn().mockResolvedValue({ ...listResult, limit: 5, offset: 10 }),
    });
...
describe("GET /api/v1/admin/feedback/summary", () => {
  const summary = {
    surveyId: "survey-1",
    respondentCount: 2,
    tallies: [{ questionId: "q-1", choiceValue: "very_disappointed", count: 2 }],
  };
  ...
  it("returns 200 with the tallies for an admin", async () => {
    ...
    const res = await app.request("/api/v1/admin/feedback/summary?surveyId=survey-1");
