...
function isAnswerContractError(error: unknown): error is Error {
  return ANSWER_CONTRACT_ERRORS.some((errorClass) => error instanceof errorClass);
}
...
export class FeedbackController {
  ...
  getActiveSurvey: RouteHandler<typeof getActiveFeedbackSurveyRoute, AppEnv> = async (c) => {
    try {
      ...
      if (error instanceof ActiveFeedbackSurveyNotFoundError) {
        return errorResponse(c, 404, ErrorCodes.FEEDBACK_SURVEY_NOT_FOUND, error.message);
      }
      throw error;
    }
  };
  ...
  submitFeedback: RouteHandler<typeof submitFeedbackRoute, AppEnv> = async (c) => {
    ...
    try {
      ...
      if (isAnswerContractError(error)) {
        return errorResponse(c, 400, ErrorCodes.FEEDBACK_INVALID_ANSWER, error.message);
      }
  ...
  listSubmissions: RouteHandler<typeof listFeedbackSubmissionsRoute, AppEnv> = async (c) => {
  ...
  getSummary: RouteHandler<typeof summarizeFeedbackRoute, AppEnv> = async (c) => {
