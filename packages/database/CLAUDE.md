...
- **接続先の分離**: 結合テストでは `TEST_DATABASE_URL` を必須とし、データベース名が `_test` で終わる専用 DB だけを使う。`DATABASE_URL`（開発 DB）へのフォールバックは禁止。テスト DB の作成と `db:test:migrate:deploy` は明示的に実行し、テストハーネスから DB 作成やマイグレーションは行わない。
