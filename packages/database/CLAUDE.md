...
- **標準: 実 DB への結合テスト**。repository / query-service の実装と同じ階層に `*.test.ts` を co-located で置き、実 DB（docker の Postgres + wsproxy、`createPrismaClient(url, { localProxy: true })` 経由）に対して実行する。1 実装につき最低限「往復（save → findById で復元一致）」「null 系」「制約違反系」をカバーすること。
